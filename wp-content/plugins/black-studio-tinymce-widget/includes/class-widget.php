	...
	class WP_Widget_Black_Studio_TinyMCE extends WP_Widget {
		...
		public function widget( $args, $instance ) {
			...
			if ( ! ( $hide_empty && empty( $text ) ) ) {
				?>
					<div class="col large-blog">
            <div class="content-image-box2">
                          <div class="img-box">
                              <img src="<?php echo $imageurl = wp_get_attachment_url(@$instance['image']); ?>" alt="">
                            </div>
                            <div class="content">
		...
		public function creatingImageUlpoadFields($instance,$key,$imagePlaceHolderId,$label) {
     	...
     	$imageurl = wp_get_attachment_url(@$instance['image']);
     	?>
     	<p class="cecp-widget-image">
     		...
     		<input name="<?php echo $key; ?>" id="<?php echo $this->get_field_id( $key ); ?>" class="widefat <?php echo $imagePlaceHolderId; ?>" type="hidden" size="36"  value="<?php echo  (int)@$instance['image'] ; ?>" />

     		<input class="upload_image_button button button-primary" type="button" value="Upload Image"  data-holder-id="<?php echo $imagePlaceHolderId; ?>" id="testjust" />
     	</p>
     	<?php
		...
		public function form( $instance ) {
			global $wp_customize;
			$instance = wp_parse_args( (array) $instance, array( 'title' => '', 'text' => '', 'type' => 'visual','link' => '' ) );
			...
			<p><label for="<?php echo esc_attr( $this->get_field_id( 'link' ) ); ?>"><?php _e( 'Link:' ); ?></label>
            ...
            <?php
            $this->creatingImageUlpoadFields($instance,$this->get_field_name('image'),$this->get_field_id('image'),"widget image");
		}
